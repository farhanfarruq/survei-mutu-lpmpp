...
class SurveyTemplatePolicy
{
    ...
    {
        return $user->can('template.read') && $this->scope->allows($user, $template->owner_unit_id);
    }
    ...
    {
        return $user->can('template.update') && $this->scope->allows($user, $template->owner_unit_id);
    }

    public function delete(User $user, SurveyTemplate $template): bool
    {
        return $user->can('template.delete') && $this->scope->allows($user, $template->owner_unit_id) && ! $template->versions()->whereHas('surveys')->exists();
    }
}
