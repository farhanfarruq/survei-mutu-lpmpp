...
{
    public function request(AnalysisRun $run, AiProviderConfig $config, AiPromptTemplate $template, User $requester): AiJob
    {
    ...
    public function baseUrlAllowed(string $provider, string $url): bool
    {
        ...
        return in_array($normalized, array_map(fn ($allowed) => rtrim(strtolower($allowed), '/'), config('ai.allowed_base_urls.'.strtolower($provider), [])), true);
    }
}
