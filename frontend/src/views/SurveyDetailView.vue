...
async function start() {
  ...
  error.value = ''
  try {
    const { credentials, response } = await startOrResumeAuthenticatedResponse(survey.value.id)
...
<template>
  <section class="response-page" aria-labelledby="survey-title">
    ...
    <BaseAlert v-else-if="error && !survey" tone="error" title="Survei tidak dapat dibuka">{{
      error
    }}</BaseAlert>
    <template v-else-if="survey">
      <div class="page-heading">
        <div>
          ...
          <h1 id="survey-title" tabindex="-1">{{ survey.name }}</h1>
          <p class="lede">
      ...
      <div class="detail-grid">
        <article class="panel">
          ...
          <dl class="definition-list">
            <div>
              <dt>Batas waktu</dt>
              <dd>{{ new Date(survey.closes_at).toLocaleString('id-ID') }}</dd>
            </div>
            <div>
              <dt>Partisipasi</dt>
              <dd>Sukarela; Anda dapat berhenti sebelum mengirim.</dd>
            </div>
            <div>
              <dt>Mode privasi</dt>
              <dd>
                {{
                  survey.privacy_mode === 'confidential'
                    ? 'Rahasia dengan akses terbatas'
                    : 'Isi jawaban dipisahkan dari identitas'
                }}
              </dd>
            </div>
          </dl>
        ...
        <article class="panel privacy-panel">
          <div>
            ...
            <p class="fine-print">
              Pimpinan hanya menerima hasil agregat yang telah melewati threshold pelaporan, bukan
              jawaban individual.
            </p>
          </div>
        </article>
      ...
      <div class="consent-panel panel">
        ...
        <div class="button-row">
          ...
          ><RouterLink class="button button-secondary" to="/app/surveys">Belum sekarang</RouterLink>
        </div>
      </div>
    </template>
  </section>
